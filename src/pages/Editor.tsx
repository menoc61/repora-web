      ...
      <header className="fixed top-0 right-0 w-[calc(100%-var(--sidebar-width,280px))] h-16 bg-surface-studio border-b border-outline-variant flex justify-between items-center px-gutter z-40">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2 ml-2 pl-4 border-l border-outline-variant">
            ...
            <Link to="/history" className="text-on-surface-variant hover:text-primary" title="Historique des versions"><Icon name="history" /></Link>
      ...
      <section className="flex-1 bg-white overflow-y-auto hide-scrollbar relative">
        <div className="max-w-[800px] mx-auto py-20 px-12 min-h-full">
          <div className="mb-12">
            ...
            <div className="flex items-center gap-4 text-on-surface-variant">
              <span className="font-label-sm text-label-sm bg-surface-container px-2 py-1 rounded">INTERNE UNIQUEMENT</span>
              <span className="font-label-sm text-label-sm">Modifie il y a 4 minutes par l&apos;Orchestrateur IA</span>
            </div>
          </div>

          <div className="space-y-6 font-body-md text-on-surface leading-relaxed">
            {isLoading && <p className="text-on-surface-variant">Chargement du document...</p>}
            {!isLoading && document && (
              document.content ? (
                ...
                <div className="p-8 border-2 border-dashed border-outline-variant rounded-xl flex flex-col items-center justify-center text-on-surface-variant">
                  ...
                  <span className="font-label-md text-label-md">Aucun contenu. Generez ce document depuis l&apos;espace de travail pour le rediger avec l&apos;Orchestrateur IA.</span>
                </div>
              )
            )}
            <div className="mt-12 p-8 border-2 border-dashed border-outline-variant rounded-xl flex flex-col items-center justify-center text-on-surface-variant hover:border-ai-vibrant hover:bg-ai-glow/10 transition-all cursor-pointer group">
              ...
              <span className="font-label-md text-label-md">Cliquez ou glissez pour inserer un nouveau bloc</span>
            </div>
          </div>
        </div>
        ...
        <div className="fixed bottom-10 left-1/2 -translate-x-1/2 bg-white shadow-xl border border-outline-variant rounded-full px-4 py-2 flex items-center gap-4 z-30">
          <Button variant="ghost" className="flex items-center gap-2 text-ai-vibrant font-bold text-label-md border-r border-outline-variant pr-4">
            <Icon name="auto_awesome" className="text-[18px]" /> Demander a l&apos;IA
          </Button>
      ...
      <aside className="fixed right-0 top-16 h-[calc(100vh-64px)] w-inspector-width bg-surface border-l border-outline-variant flex flex-col z-40 overflow-hidden">
        <div className="p-gutter border-b border-outline-variant">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="bg-ai-glow text-ai-vibrant font-label-sm text-label-sm px-2 py-0.5 rounded">3 ACTIFS</span>
          </div>
          <div className="space-y-4">
            <AgentStatus name="ARCHITECT" state="idle"><p className="text-[12px] text-on-surface-variant">Structure du document optimisee pour la lisibilite executive.</p></AgentStatus>
            <div className="bg-white p-3 rounded border-2 border-ai-vibrant shadow-md">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="font-label-sm text-[10px] text-ai-vibrant">REDACTION...</span>
              </div>
              <div className="space-y-1">
                ...
                <p className="text-[12px] text-on-surface">Developpement de la section : Analyse regionale APAC...</p>
              </div>
            </div>
            <AgentStatus name="CRITIC" state="review"><p className="text-[12px] text-on-surface-variant">En file d&apos;attente : Verification des faits et audit des citations.</p></AgentStatus>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-gutter bg-surface-studio">
          ...
          <nav className="space-y-3">
            <OutlineItem label="Resume executif" done />
            <OutlineItem label="Facteurs cles du marche" active sub={['Dynamiques regionales', 'Cadre reglementaire', 'Evolutions technologiques']} />
            <OutlineItem label="Projections fiscales" />
            <OutlineItem label="Attenuation des risques" />
          </nav>
        </div>
