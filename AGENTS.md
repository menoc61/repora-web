...
- **`LlamaCppProvider`** — talks to a local `llama-server` instance over its OpenAI-compatible `/v1/chat/completions` endpoint (`stream: true`). Default provider, zero configuration, zero API cost, data never leaves the box. Any GGUF model the operator has downloaded can be swapped in via config (no code change) — a Hermes-family tool-calling model (or any function-calling-capable GGUF) is the recommended default given the orchestrator's tool-use needs.
  ...
  **Model discovery & tool-call detection:** at startup the backend enumerates every model available via `ollama list` (falling back to the Ollama HTTP `/api/tags`). The default model is `nemotron-3-super:cloud` (override with the `OLLAMA_MODEL` env var). Only the default model is probed for tool-calling support at boot — the remaining models are probed lazily on first use — so a retired/forbidden cloud model can never block startup. Probing sends a minimal request containing a dummy tool and records whether the model emits a `tool-call`; results are cached (`toolSupportCache`). A name-based heuristic (`knownToolCallers` in `providers/interface.ts`, which includes `nemotron`, `kimi`, `minimax`, `glm`, `granite`, plus the usual GPT/Claude/Gemini/Qwen/Llama families) is used as a fallback before a model has been probed. If an agent is handed tools but the model produces zero output or errors, `runAgent` transparently retries without tools. Process-level `unhandledRejection`/`uncaughtException` guards ensure one flaky provider call cannot crash the backend.
...
services:
  ...
  db:            # postgres:17 with healthcheck (pg_isready), localhost:5434
```
